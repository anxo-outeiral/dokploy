...
const cleanupOldVolumeBackups = async (
	...
	try {
		...
		const listCommand = `rclone lsf ${rcloneFlags.join(" ")} --include \"${volumeName}-*.tar.zst\" ${backupFilesPath}`;
...
export const runVolumeBackup = async (volumeBackupId: string) => {
	...
	try {
		...
		// delete all the .tar.zst files
		const command = `rm -rf ${volumeBackupPath}/*.tar.zst`;
		if (serverId) {
